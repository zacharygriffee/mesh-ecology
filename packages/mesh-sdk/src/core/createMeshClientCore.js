...
async function joinTopic(swarm, topicBuf, sleep) {
  ...
  if (handle && typeof handle.flushed === "function") {
    await Promise.race([handle.flushed(), sleep(60)]).catch(() => {});
  }
  return handle;
}
...
function assertPlatform(platform) {
  if (!platform || typeof platform !== "object") throw new Error("platform is required");
  if (typeof platform.resolveStoreRoot !== "function") throw new Error("platform.resolveStoreRoot is required");
  if (typeof platform.loadMeshRuntime !== "function") throw new Error("platform.loadMeshRuntime is required");
  if (typeof platform.sleep !== "function") throw new Error("platform.sleep is required");
  if (typeof platform.nowMs !== "function") throw new Error("platform.nowMs is required");
...
function createMeshClientCore(platform, config = {}, options = {}) {
  ...
  let runtimeOpsPromise = null;

  function assertRuntimeOps(runtimeOps) {
    if (!runtimeOps || typeof runtimeOps !== "object") throw new Error("platform.loadMeshRuntime() must return an object");
    if (typeof runtimeOps.ensureCorestore !== "function") throw new Error("runtime.ensureCorestore is required");
    if (typeof runtimeOps.ensureDiscoverySurface !== "function") throw new Error("runtime.ensureDiscoverySurface is required");
    if (typeof runtimeOps.ensureConcernSurface !== "function") throw new Error("runtime.ensureConcernSurface is required");
    if (typeof runtimeOps.getJobView !== "function") throw new Error("runtime.getJobView is required");
    if (typeof runtimeOps.getPublishView !== "function") throw new Error("runtime.getPublishView is required");
    if (typeof runtimeOps.getRatView !== "function") throw new Error("runtime.getRatView is required");
    if (typeof runtimeOps.publishJobWork !== "function") throw new Error("runtime.publishJobWork is required");
    if (typeof runtimeOps.publishJobRatification !== "function") throw new Error("runtime.publishJobRatification is required");
    if (!runtimeOps.KIND || typeof runtimeOps.KIND !== "object") throw new Error("runtime.KIND is required");
  }

  async function getRuntimeOps() {
    if (!runtimeOpsPromise) {
      runtimeOpsPromise = Promise.resolve(platform.loadMeshRuntime()).then((runtimeOps) => {
        assertRuntimeOps(runtimeOps);
        return runtimeOps;
      });
    }
    return runtimeOpsPromise;
  }

  async function ensureReady() {
    if (corestore) return;
    const runtime = await getRuntimeOps();
    corestore = runtime.ensureCorestore(storeRoot);
  ...
  async function getDiscoveryBase() {
    ...
    discoveryBase = await runtime.ensureDiscoverySurface(corestore.namespace(namespace), { key: keyBuf }, swarm);
    await joinTopic(swarm, discoveryBase.discoveryKey, delay);
  ...
  async function getConcernBase(concernKey) {
    ...
    const base = await runtime.ensureConcernSurface(corestore.namespace(namespace), swarm, { key: keyBuf });
    concernBases.set(keyZ32, base);
    await joinTopic(swarm, base.discoveryKey, delay);
  ...
  async function scanDiscoveryConcerns() {
    ...
    for await (const row of disc.view.createReadStream()) {
      if (!row || row.t !== runtime.KIND.CONCERN || !row.k32) continue;
  ...
  async function collectState({ overrideConcernKeys = null } = {}) {
    ...
    for (const target of targets.concerns) {
      ...
      try {
        ...
        concernsRow.counts.jobs = await countEntries(runtime.getJobView(base));
        concernsRow.counts.pubs = await countEntries(runtime.getPublishView(base));
        concernsRow.counts.rats = await countEntries(runtime.getRatView(base));
  ...
  async function trace({
    ...
    for (const concernState of baseState.concerns) {
      ...
      try {
        ...
        const jobView = runtime.getJobView(base);
        const publishView = runtime.getPublishView(base);
        const ratView = runtime.getRatView(base);
  ...
  async function proposePub({ concernKey, cap, ref, meta } = {}) {
    try {
      ...
      await runtime.publishJobWork(base, jobKeyBuf, capValue, normalizedRef, meta);
  ...
  async function proposeRat({
    ...
    try {
      ...
      await runtime.publishJobRatification(
        base,
        jobKeyBuf,
        orgBuf,
        attemptBuf,
  ...
  async function waitForMaterialization({
    ...
    while (platform.nowMs() - start <= maxMs) {
    ...
    return {
      ...
      concernKey: concernZ32,
      jobKey: jobZ32,
      elapsedMs: platform.nowMs() - start
    };
  }
  ...
  function watchState({ intervalMs = 1000 } = {}, onState) {
    ...
    const stopTimer = platform.scheduleInterval(tick, everyMs);
    void tick();
    return () => {
      closed = true;
      stopTimer();
